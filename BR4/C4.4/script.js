...
function renderMessages(messages){ if(!messages.length){ chatDiv.innerHTML='<div class="loading">История пуста. Напишите первое сообщение.</div>'; return; } chatDiv.innerHTML=""; for(const msg of messages){ const div = document.createElement("div"); div.className=`message ${msg.role}`; div.textContent=msg.content; chatDiv.appendChild(div); } chatDiv.scrollTop=chatDiv.scrollHeight; }
function addMessageToChat(role, content){ const div = document.createElement("div"); div.className=`message ${role}`; div.textContent=content; chatDiv.appendChild(div); chatDiv.scrollTop=chatDiv.scrollHeight; }
